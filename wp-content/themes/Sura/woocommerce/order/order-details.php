...
<table class="shop_table order_details table">
							...
							<?php
								...
								if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
									...
									foreach ( $download_files as $download_id => $file ) {
										...
										$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', 'teo' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
									}
	...
	<?php
				...
				if ( $has_refund && $key === 'order_total' ) {
					...
					if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
						...
						if ( ! empty( $tax_del_array ) ) {
							$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'teo' ), implode( ', ', $tax_del_array ) );
						}

						if ( ! empty( $tax_ins_array ) ) {
							$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'teo' ), implode( ', ', $tax_ins_array ) );
						}
					}
		...
		if ( $has_refund ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Refunded:', 'teo' ); ?></th>
		...
		if ( '' != $order->customer_note ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Note:', 'teo' ); ?></th>
...
<header>
	<h3><?php _e( 'Customer details', 'teo' ); ?></h3>
</header>
<dl class="customer_details">
<?php
	if ( $order->billing_email ) echo '<dt>' . __( 'Email:', 'teo' ) . '</dt><dd>' . $order->billing_email . '</dd>';
	if ( $order->billing_phone ) echo '<dt>' . __( 'Telephone:', 'teo' ) . '</dt><dd>' . $order->billing_phone . '</dd>';
...
<?php endif; ?>
		...
		<header class="title">
			<h3><?php _e( 'Billing Address', 'teo' ); ?></h3>
		</header>
		<address>
			<?php
				if ( ! $order->get_formatted_billing_address() ) _e( 'N/A', 'teo' ); else echo $order->get_formatted_billing_address();
			?>
		</address>
...
<?php if ( ! wc_ship_to_billing_address_only() && $order->needs_shipping_address() && get_option( 'woocommerce_calc_shipping' ) !== 'no' ) : ?>
	...
	<div class="col-sm-6">
		...
		<header class="title">
			<h3><?php _e( 'Shipping Address', 'teo' ); ?></h3>
		</header>
		<address>
			<?php
				if ( ! $order->get_formatted_shipping_address() ) _e( 'N/A', 'teo' ); else echo $order->get_formatted_shipping_address();
			?>
		</address>
